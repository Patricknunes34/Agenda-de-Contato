...
void Adicionar_contato() {
    ...
    while (fgets(linha, sizeof(linha), Arquivo)) {
        char nomeExistente[50], telefoneExistente[20];
        sscanf(linha, "Nome: %49[^,], Telefone: %19[^\n]", nomeExistente, telefoneExistente);
        if (strcmp(telefoneExistente, telefone) == 0) {
            char aviso[150];
            snprintf(aviso, sizeof(aviso), "Erro: Número já existe na agenda! Dono: %s", nomeExistente);
            ALERTA(aviso);
            fclose(Arquivo);
            return;
        } else if (strstr(telefoneExistente, telefone) != NULL || strstr(telefone, telefoneExistente) != NULL) {
            char aviso[150];
            snprintf(aviso, sizeof(aviso), "Aviso: Número semelhante já existe na agenda! Dono: %s", nomeExistente);
            ALERTA(aviso);
        }
    }
    fclose(Arquivo);
